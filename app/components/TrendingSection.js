...
export default function TrendingSection({ selectedSources = [], selectedSentiment = "All", selectedDate = "" }) {
  ...
  let filteredTrending = trending.filter((post) => {
    const showAllSources = selectedSources.length === 0 || selectedSources.includes("All");
    const sourceMatch = showAllSources || selectedSources.includes(post.platform_name);
  ...
  <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-xl shadow-md p-6 mb-8 w-full">
      <h2 className="text-lg font-bold mb-4 text-indigo-900">Trending</h2>
      {/* Time Filter */}
      <div className="mb-4 flex items-center gap-2">
        ...
        <select
          ...
          className="px-2 py-1 rounded border border-gray-300 bg-gray-50 text-gray-900"
        >
      ...
      <div className="mt-2 overflow-y-auto" style={{ maxHeight: "400px" }}>
            ...
            {posts.map((post, idx) => (
              ...
              >
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-xs px-2 py-1 rounded font-semibold bg-indigo-100 text-indigo-700">
                    {post.sentiment}
                  </span>
                ...
                <div className="text-base text-gray-900">{post.title}</div>
        ...
        {activeTab === "Keywords" && (
          <div className="flex flex-wrap gap-2">
            {keywords.length === 0 && (
              <span className="text-gray-400">No keywords found.</span>
            )}
